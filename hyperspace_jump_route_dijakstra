...
def relax(u,v,weights,Q):
    if v.get_dist()>u.get_dist()+weights[u.num][v.num]:
        v.set_dist(u.get_dist() + weights[u.num][v.num])
        i = Q.index(v)
...
def hyperspace_jump_route(star_list,jump_times):
    ...
    while  len(Q)!=0:
        ...
        for i in range(len(u.adj)):
            v = relax(u,u.adj[i],jump_times,Q)
            if v!= None:
                path.insert(0, v)


    for i in range(0,len(path)):
        if path[i].color == 'orange':
            ind = i
            break
